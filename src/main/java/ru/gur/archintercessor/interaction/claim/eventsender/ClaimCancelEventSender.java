...
public class ClaimCancelEventSender implements ClaimEventSender {
    ...
    public boolean canSend(final ClaimEventType claimEventType) {
        ...
        return ClaimEventType.CANCEL.equals(claimEventType);
    }

    @Override
    public void sendEvent(final ClaimData claimData, final String processId) {
        final ClaimCancelEventData claimCancelEventData =
                ClaimCancelEventData.builder()
                        .id(claimData.getId())
                        .build();
